...
// Toutes les routes Modèles exigent une session valide.
 ...
 */
async function assertNoDuplicateNomCategorie(nom, categorie, excludeId) {
...
router.post("/", async (req, res) => {
  ...
  await assertNoDuplicateNomCategorie(data.nom, data.categorie);

  const modele = await prisma.modele.create({ data });
...
router.get("/:id", async (req, res) => {
  const modele = await prisma.modele.findUnique({ where: { id: req.params.id } });
...
router.patch("/:id", async (req, res) => {
  ...
  const current = await prisma.modele.findUnique({
    where: { id },
  ...
  if (data.nom !== undefined || data.categorie !== undefined) {
    ...
    await assertNoDuplicateNomCategorie(nom, categorie, id);
  }
  ...
  const result = await prisma.modele.updateMany({ where: { id, archivedAt: null }, data });
  if (result.count === 0) {
    throw new HttpError(409, "Modèle archivé : restaurez-le avant de le modifier.");
  }
...
router.post("/:id/archiver", async (req, res) => {
  ...
  if (result.count === 0) {
    const existing = await prisma.modele.findUnique({ where: { id }, select: { archivedAt: true } });
