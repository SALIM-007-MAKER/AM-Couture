...
import { useReinitialiserMotDePasseTokenMutation } from "../../hooks/useAuth.js";
import { GlobalFormError } from "../../components/QueryState.jsx";
import { useTranslation } from "../../i18n/index.js";

// Page atteinte depuis le lien envoyé par email (voir POST
// /mot-de-passe-oublie) — volontairement PAS sous PublicOnlyRoute/
// ProtectedRoute (voir App.jsx) : accessible que la personne soit déjà
// connectée ailleurs dans le même navigateur ou pas, le jeton dans l'URL
// authentifie l'action à lui seul.
const NOM_PLATEFORME = "Gestion d'Atelier";

export default function ReinitialiserMotDePasseTokenPage() {
  const { t } = useTranslation();
  const [searchParams] = useSearchParams();
  const token = searchParams.get("token") ?? "";
  const navigate = useNavigate();
  const mutation = useReinitialiserMotDePasseTokenMutation();
      ...
      <div className="relative w-full max-w-sm flex flex-col items-center">
        ...
        <div className="w-full rounded-3xl bg-neutral-900/70 backdrop-blur-xl border border-white/10 shadow-2xl p-6 space-y-5">
          <div className="text-center space-y-1.5">
            <h1 className="text-2xl font-semibold tracking-tight text-white">{t("auth.reset.title")}</h1>
          </div>
